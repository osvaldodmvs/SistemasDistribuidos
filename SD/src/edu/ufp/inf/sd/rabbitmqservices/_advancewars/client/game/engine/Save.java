...
/**
 * When SaveGame() is called upon it grabs a screen shot of the current battle (current_player,days) and the important variables from the player, edu.ufp.inf.sd.rmi._advancewars.client.game.buildings, and unit lists.
 * It currently saves them in a property file, with each saved variable having their own line (it gets very long fast).
 * When LoadGame() is called, it will create a new battle, change the current day and player, and then loads the edu.ufp.inf.sd.rmi._advancewars.client.game.players / edu.ufp.inf.sd.rmi._advancewars.client.game.units / cities from here.
...
public class Save {
	...
	public void SaveSettings() {
		...
		if (!directory.exists()) {
			if (directory.mkdir()) {
                ...
                edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.error.ShowError("Cannot create the save directory. " + path + "");return;}
		}
		...
		try {
		    ...
		    out.write(
		    		...
		    		"Player = Player\n" +
		    		"ScreenSize = " + edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.ScreenBase + "\n" +
		    		"TexturePack = " + "UnAddedFeature" + "\n" +
		    		"Dev = " + edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.dev + "\n" +
		    		"Volume = 0\n" +
		    		"Music = 0");
		    out.close();
		} 
		catch (Exception e) {
            edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.error.ShowError("Saving PlayerData failed:" + e.getMessage());}
	}
	public void LoadSettings() {
		try {
			...
			edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.dev = ( Boolean.parseBoolean(configFile.getProperty("Dev", "false")));
			edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.gui.ResizeScreen( Integer.parseInt(configFile.getProperty("ScreenSize", "32")));
		} 
		catch (Exception e) {
            edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.error.ShowError("Player data failed to load."); e.printStackTrace();}
	}
	...
	public void SaveGame() {
		...
		try {
		    ...
		    out.write("# A testing save file format for my advanced wars project, using property file as it is a list of properties...\n" +
		    		  "# When loading map, skip the unit stage (will load edu.ufp.inf.sd.rmi._advancewars.client.game.units from here, maybe edu.ufp.inf.sd.rmi._advancewars.client.game.buildings too with health)" +
		    		  "# I might change this to something close to the map.txt files\n" +
		    		  "Map = " + edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.btl.mapname + "\n" +
		    		  "CurrentPlayer = " + edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.btl.currentplayer + "\n" +
		    		  "Days = " + edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.btl.day + "\n" +
		    		  "Units = " + edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.units.size() + "\n");
            ...
            edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.error.ShowError("Save Failed:" + e.getMessage());}
	}
	private String SavePlayerData() {
		String plyrdata = "\n# Player Data\n";
		for (int i = 0; i < edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.btl.totalplayers; i++) {
	    	edu.ufp.inf.sd.rmi._advancewars.client.game.players.Base ply = edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.player.get(i);
	    	plyrdata+="Player" + i + "_Type = " + ply.name + "\n" +//This is the character (commander) that said player is using. (Using name to find them)
	    			  ...
	    			  "Player" + i + "_Kills = " + ply.kills + "\n" +//How many edu.ufp.inf.sd.rmi._advancewars.client.game.units they've killed
	    			  "Player" + i + "_Loses = " + ply.loses + "\n" +//How many edu.ufp.inf.sd.rmi._advancewars.client.game.units they've lost
	...
	private String SaveCityData() {
		...
		for (int i = 0; i < edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.builds.size(); i++) {
	    	edu.ufp.inf.sd.rmi._advancewars.client.game.buildings.Base bld = edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.builds.get(i);
	...
	private String SaveUnitData() {
	    ...
	    for (int i = 0; i < edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.units.size(); i++) {
	    	edu.ufp.inf.sd.rmi._advancewars.client.game.units.Base unit = edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.units.get(i);
	...
	public void LoadGame() {
		try {
			...
			edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.btl.NewGame(configFile.getProperty("Map"));
			edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.btl.currentplayer = Integer.parseInt(configFile.getProperty("CurrentPlayer"));
			edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.btl.day = Integer.parseInt(configFile.getProperty("Days"));
			...
			//Changes the edu.ufp.inf.sd.rmi._advancewars.client.game.gui to match the appropriate bla bla bla
			edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.gui.InGameScreen();
		} 
		catch (Exception e) {
            edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.error.ShowError("Saved game failed to load."); e.printStackTrace();}
	}
	private void LoadPlayers(Properties configFile) {
		for (int i = 0; i < edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.btl.totalplayers; i++) {
			edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.list.LoadCommander(
	...
	private void LoadCities(Properties configFile) {
		for (int i = 0; i < edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.builds.size(); i++) {
			edu.ufp.inf.sd.rmi._advancewars.client.game.engine.Game.list.LoadCity(
